...
int getAltura(No *raiz, int v[]){
    if (raiz!=NULL){
            int left = getAltura(raiz->left, v);  // chamada recursiva para a esqueda
            int right = getAltura(raiz->right, v); // chamada recursiva para a direita
            if (left>right){
                v[left] = raiz->chave;
