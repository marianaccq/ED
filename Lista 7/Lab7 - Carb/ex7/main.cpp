...
void in_ordem_aux(No *raiz, int &cont){

    if (raiz->left !=NULL) in_ordem_aux(raiz->left,cont);
    if(raiz->left == NULL && raiz->right == NULL){
        cont++;
    }
    if (raiz->right !=NULL) in_ordem_aux(raiz->right,cont);

}

int in_ordem(No *raiz){
    int cont=0;
    if (raiz!=NULL) in_ordem_aux(raiz,cont);
    return cont;
}
