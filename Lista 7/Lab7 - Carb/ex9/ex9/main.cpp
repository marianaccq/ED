...
void Enfileira(No *raiz, TipoFila *fila)
{
  TipoNo *aux;
...
void buscaABB_recursiva_rev(No *raiz, No **pt_retorno, int x, int &f){
    if (raiz!=NULL){
        if (raiz->chave == x){
            *pt_retorno = raiz; // pt_retorno agora aponta apra o elemento encontrado
            f = 1;
            ...
            if (x > raiz->chave ){
                if (raiz->left == NULL) f=2; // volta na recursao.
                    ...
                    *pt_retorno = raiz->left; // essa linha é importante porque a busca,
                    // caso nao encontre o nó desejado, precisa retornar o elemento que seria pai do elemento procurado.
                    buscaABB_recursiva_rev(raiz->left,pt_retorno, x, f); // Chama recursivamente para a esquerda
                }
            } else{
                if (raiz->right == NULL) f=3; // volta na recursao.
                else {
                    *pt_retorno = raiz->right; // idem
                    buscaABB_recursiva_rev(raiz->right,pt_retorno, x, f);// chama recursivamente para a direita
                }
            }
        }
    }
...
*/
void insert(int x, bool flag, No **raiz){
    int f=0;
    No *pt_retorno = *raiz;
    if(!flag){
        ...
        buscaABB_recursiva_rev(*raiz, &pt_retorno, x, f); // NOTEM NOTEM NOTEM NOTEM NOTEM NOTEM NOTEM o &pt_retorno e o *raiz
    }
...
No *reverte(No *raiz){
    ...
    do{
        raiz = Desenfileira(F);
        insert(raiz->chave,1,&rev);
        if(raiz->left!=NULL){
            Enfileira(raiz->left,F);
        }
        if(raiz->right != NULL){
            Enfileira(raiz->right,F);
        }
    }while(!filaVazia(F));
    return rev;
}
